...
import { getSportById } from "@/lib/kit-builder-config";
import {
    ArrowLeft, Send, User, Mail, Phone, Users, Hash,
    Palette, FileText, Upload, CheckCircle, Loader2, Shield
} from "lucide-react";
import { submitKitInquiry } from "@/app/actions/kitBuilderActions";
...
import { JerseyPreview } from "@/components/products/JerseyPreview";
import { JerseyPreview3D } from "@/components/products/JerseyPreview3D";
import {
    ChevronDown, ChevronUp, Check,
    RotateCcw, ShoppingBag, ArrowLeft, Save, X
} from "lucide-react";
import { saveDesignAction } from "@/app/actions/kitBuilderActions";
        ...
        return (
            <main className="min-h-screen bg-background flex items-center justify-center">
                <div className="text-center">
                    ...
                    <Link href="/kit-builder" className="text-primary text-sm mt-4 inline-block hover:underline">
                        ← Back to Kit Builder
                    </Link>
                </div>
            </main>
                    ...
                    {submitted ? (
                        ...
                        >
                            ...
                            <p className="text-white/40 text-sm max-w-md mx-auto mb-8">
                                Thank you for your interest! Our team will review your kit requirements and get back to you within 24-48 hours.
                            </p>
                        ...
                        >
                            ...
                            <div className="text-center mb-8 md:mb-10">
                                <h2 className="text-2xl md:text-3xl font-display font-black text-white uppercase tracking-tight mb-2">
                                    {sport.name} <span className="text-primary">Kit Inquiry</span>
                                </h2>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <SignedIn>
                            <button
                                ...
                                className="flex-1 md:flex-none bg-white/10 text-white border border-white/20 font-bold uppercase text-xs tracking-widest px-6 py-3 rounded hover:bg-white hover:text-black transition flex items-center justify-center gap-2"
                            >
                        ...
                        <button
                            ...
                            className="flex-1 md:flex-none bg-primary text-black font-bold uppercase text-xs tracking-widest px-6 py-3 rounded hover:bg-primary/90 transition flex items-center justify-center gap-2"
                        >
                ...
                {isSaveModalOpen && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-[#111] border border-white/10 p-6 md:p-8 rounded-xl w-full max-w-md shadow-2xl relative"
                        >
                            <button
                            ...
                            <h3 className="text-xl font-display font-black uppercase text-white mb-2">Save Design</h3>
                            <p className="text-sm text-muted mb-6">Give your design a name so you can easily find it in your profile later.</p>

                            <form onSubmit={handleSaveDesign} className="space-y-4">
                                <div>
                                    <label className="text-xs font-bold uppercase tracking-widest text-muted mb-2 block">Design Name</label>
                                    <input
                                        type="text"
                                        value={designName}
                                        onChange={e => setDesignName(e.target.value)}
                                        placeholder="e.g., Match Day Kit 2026"
                                        className="w-full bg-white/5 border border-white/10 rounded px-4 py-3 text-white text-sm focus:border-primary focus:outline-none"
                                ...
                                {saveError && (
                                    <div className="text-red-400 text-sm font-medium">{saveError}</div>
                                )}

                                <button
                                    type="submit"
                                    disabled={isPending || saveSuccess || !designName.trim()}
                                    className={`w-full py-3 rounded font-bold uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 ${saveSuccess
                                        ? "bg-green-500 text-white"
                                        : "bg-primary text-black hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
                                        }`}
                                >
                                    {isPending ? "Saving..." : saveSuccess ? <><Check className="w-4 h-4" /> Saved!</> : "Save to Profile"}
                                </button>
                            </form>
                        </motion.div>
                    </motion.div>
